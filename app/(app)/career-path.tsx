  ...
  return (
    <AppCard>
      <View style={{ gap: 12 }}>
        ...
        <View style={{ gap: 8 }}>
          {steps.slice(0, 3).map((step, index) => <Text key={`${index}-${step}`} style={{ color: colors.text, lineHeight: 22 }}>• {step}</Text>)}
        </View>
...
export default function CareerPathScreen() {
  ...
  if (loading && !activeData) return <LoadingView label="Loading your path..." />;

  return (
    <AppScreen>
      <Text style={{ fontSize: 30, fontWeight: '800', color: colors.text }}>Career Path</Text>
      ...
      <AppCard>
        <View style={{ gap: 14 }}>
          ...
          <AppSelect
            ...
            onChange={(value) => setAnswers((prev) => ({ ...prev, yearsExperience: value }))}
          ...
          <AppSelect
            ...
            onChange={(value) => setAnswers((prev) => ({ ...prev, englishLevel: value }))}
          ...
          />
          <AppButton label={uploadMutation.isPending ? 'Uploading resume...' : uploadedFileName ? `Resume attached: ${uploadedFileName}` : 'Attach resume to improve path accuracy'} variant="secondary" onPress={() => void handlePickDocument()} />
          {uploadMessage ? <Text style={{ color: colors.muted }}>{uploadMessage}</Text> : null}
          <AppButton label={generateMutation.isPending ? 'Generating path...' : 'Generate my path'} onPress={() => generateMutation.mutate()} disabled={generateMutation.isPending || !answers.profession.trim()} />
        </View>
      </AppCard>
      ...
      {!activeData ? <EmptyState title="No path yet" message="Generate your path to see the strongest route, a bridge route, and the first steps to take." /> : null}
      ...
      {activeData ? (
        <>
          <PathCard
            ...
            onPress={() => setAnswers((prev) => ({ ...prev, targetGoal: activeData.primaryPath.title }))}
          />
          ...
          <PathCard
            ...
            onPress={() => setAnswers((prev) => ({ ...prev, targetGoal: activeData.bridgePath?.title || prev.targetGoal }))}
          />
          ...
          <PathCard
            ...
            steps={(activeData.skillsToBuild?.slice(0, 3).map((item) => `Build or show ${item}`) || []).concat(['Track applications weekly', 'Prepare one simple interview story'])}
            cta="Use this as my urgent route"
            onPress={() => setAnswers((prev) => ({ ...prev, urgencyLevel: 'high' }))}
          />

          {activeData.skillsToBuild?.length ? (
            <AppCard>
              <Text style={{ fontSize: 18, fontWeight: '800', color: colors.text }}>Skills to build next</Text>
              <View style={{ marginTop: 10, gap: 8 }}>
                {activeData.skillsToBuild.map((item, index) => <Text key={`${index}-${item}`} style={{ color: colors.muted, lineHeight: 22 }}>• {item}</Text>)}
              </View>
            </AppCard>
          ) : null}
          ...
          {activeData.reasoning ? (
            <AppCard>
              <Text style={{ fontSize: 18, fontWeight: '800', color: colors.text }}>Why this path fits you</Text>
              <Text style={{ marginTop: 10, color: colors.muted, lineHeight: 24 }}>{activeData.reasoning}</Text>
            </AppCard>
          ) : null}

          {activeData.regulatedWarning ? (
            <AppCard>
              <Text style={{ fontSize: 18, fontWeight: '800', color: colors.text }}>Important note</Text>
              <Text style={{ marginTop: 10, color: colors.muted, lineHeight: 24 }}>{activeData.regulatedWarning}</Text>
            </AppCard>
          ) : null}

          {activeData.officialLinks?.length ? (
            <AppCard>
              <Text style={{ fontSize: 18, fontWeight: '800', color: colors.text }}>Official links</Text>
