...
const STATIC_API_BASE_URL = env.apiBaseUrl;
...
export default function LiveInterviewSessionScreen() {
  ...
  const stopAllAudio = useCallback(async function stopAllAudio() {
    try {
      Speech.stop();
  ...
  const speakLocally = useCallback(async function speakLocally(text?: string | null) {
  ...
  const playCoachVoice = useCallback(async function playCoachVoice(audioUrl?: string | null, fallbackText?: string | null) {
    if (!voiceEnabled) return;
    const fullUrl = audioUrl ? (audioUrl.startsWith('http') ? audioUrl : `${STATIC_API_BASE_URL}${audioUrl}`) : null;
  ...
  }, [currentQuestion, params.audioUrl, playCoachVoice, stopAllAudio]);
