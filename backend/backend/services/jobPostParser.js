...
function looksBlocked(text = '') {
  const s = String(text || '');
  return /captcha|access denied|sign in to continue|unusual traffic|enable javascript|forbidden|cloudflare/i.test(s);
}
...
async function fetchWithFallback(url) {
  ...
  if (direct.ok && direct.body && direct.body.length > 1200 && !looksBlocked(direct.body)) {
  ...
  for (const mirrorUrl of mirrors) {
    try {
      ...
      if (mirrored.ok && mirrored.body && mirrored.body.length > 300 && !looksBlocked(mirrored.body)) {
...
function extractDescription($, platform, rawFallback = '') {
  const selectorsByPlatform = {
    linkedin: ['.show-more-less-html__markup', '.description__text', '.jobs-description__container', '.jobs-box__html-content', 'main'],
    greenhouse: ['#content', '.content', '#app-body', 'main', 'article'],
    lever: ['.section-wrapper.page-full-width', '.posting-page', '.content', 'main', 'article'],
    workday: ['[data-automation-id="jobPostingDescription"]', '[data-automation-id="externalPosting"]', 'main', 'article'],
    indeed: ['#jobDescriptionText', '[data-testid="jobsearch-JobComponent-description"]', 'main', 'article'],
    generic: ['[data-testid*="jobDescription"]', '[class*="job-description"]', '[class*="jobDescription"]', '[class*="description"]', 'main', 'article', 'body']
  };
  ...
  if (candidateBlocks[0] && candidateBlocks[0].length > 250) return candidateBlocks[0];

  const embedded = extractEmbeddedJobData($);
  if (embedded.description && embedded.description.length > 250) return cleanDescriptionText(embedded.description);

  const metaDescription = pickMeta($, ['meta[name="description"]', 'meta[property="og:description"]', 'meta[name="twitter:description"]']);
  if (metaDescription && metaDescription.length > 120) return cleanDescriptionText(metaDescription);

  return cleanDescriptionText(rawFallback || $.root().text());
}
...
function trimMirrorText(raw = '') {
  ...
  return cleanDescriptionText(sectionsOnly.join('\n'));
}
...
async function refineExtractedJobPosting({ url = '', platform = 'generic', source = '', extractionMethod = '', title = '', company = '', location = '', salary = '', text = '' }) {
  ...
  if (!cleaned || cleaned.length < 240) {
    return {
      ...
      text: cleaned,
      responsibilities: extractedLists.responsibilities,
      requirements: extractedLists.requirements,
      skills: extractedLists.skills,
  ...
  try {
    ...
    if (out) {
      ...
      return {
        ...
        text: refinedText,
        responsibilities: Array.isArray(parsed?.responsibilities) ? parsed.responsibilities.map(cleanText).filter(Boolean).slice(0, 10) : extractedLists.responsibilities,
        requirements: Array.isArray(parsed?.requirements) ? parsed.requirements.map(cleanText).filter(Boolean).slice(0, 10) : extractedLists.requirements,
        skills: Array.isArray(parsed?.skills) ? parsed.skills.map(cleanText).filter(Boolean).slice(0, 15) : extractedLists.skills,
...
async function parseJobPostingUrl(url) {
  ...
  if ((!fetched.html || fetched.html.length < 1200 || fetched.source === 'direct-partial' || fetched.source === 'direct-error-body' || looksBlocked(fetched.html)) && platform !== 'generic') {
    try {
  ...
  if (fetched.source === 'jina-mirror') {
    ...
    return {
      ...
      warning: refined.warning || 'This site appears heavily client-rendered or access-restricted. Review the extracted text before tailoring.'
    };
  }
  ...
  const description = cleanText(jobPosting?.description ? stripHtml(jobPosting.description) : (embedded.description || extractDescription($, platform, fetched.browserText || html)));
  ...
  const warning = refined.warning || ((!refined.text || refined.text.length < 300)
    ? 'JobNova found only limited job text from this page. Paste the missing duties, skills, and qualifications manually if the posting looks incomplete.'
  ...
  return {
    ...
    text: (refined.text || description).slice(0, 24000),
    responsibilities: refined.responsibilities || [],
    requirements: refined.requirements || [],
    skills: refined.skills || [],
