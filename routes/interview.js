...
async function createRealtimeClientSecret(payload = {}) {
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/realtime/client_secrets', {
      ...
      body: JSON.stringify({
        ...
        session: {
          ...
          instructions: `You are JobNova's recruiter-style interview coach. Conduct a realistic live interview for the role ${payload.targetRole || 'the target role'}. Ask one question at a time, do not interrupt, wait until the candidate finishes speaking, ask natural follow-up questions, and keep feedback concise until the final report.`,
          audio: {
            input: {
            ...
            output: {
              voice: payload.recruiterVoice || 'marin'
            }
          }
        }
      })
    ...
    return {
      clientSecret: data?.client_secret?.value || data?.value || null,
      realtimeSessionId: data?.session?.id || data?.id || null,
      realtimeModel: data?.session?.model || data?.model || process.env.OPENAI_REALTIME_MODEL || 'gpt-realtime',
      realtimeTransport: 'webrtc',
      realtimeReady: Boolean(data?.client_secret?.value)
    };
  } catch {
    return null;
  }
...
router.post('/live/start', async (req, res) => {
  try {
    ...
    const voice = await maybeMakeVoice(session.currentQuestion, session.payload?.recruiterVoice || 'marin');
    const realtime = await createRealtimeClientSecret(session.payload || {});
    if (!canRespond(req, res)) return;
    return res.json({ sessionId: session.sessionId, firstQuestion: session.currentQuestion, totalQuestions: session.totalQuestions, ...voice, ...(realtime || { realtimeReady: false, realtimeTransport: null, clientSecret: null, realtimeSessionId: null, realtimeModel: null }) });
  } catch (error) {
    if (!canRespond(req, res)) return;
...
router.post('/live/respond', upload.single('audio'), async (req, res) => {
  try {
    ...
    const voice = await maybeMakeVoice(result.coachReply + (result.nextQuestion ? ` ${result.nextQuestion}` : ''), session.payload?.recruiterVoice || 'marin');
    if (!canRespond(req, res)) return;
