...
router.post('/feedback', async (req, res) => {
  try {
    ...
    saveState();
    await syncUserState(req.user, req.userData, 'interview_feedback', saved, saved.id);
...
router.post('/live/start', async (req, res) => {
  try {
    ...
    saveState();
    await syncUserState(req.user, req.userData, 'live_interview_start', req.userData.liveSessions[session.sessionId], session.sessionId);
...
router.post('/live/respond', upload.single('audio'), async (req, res) => {
  try {
    ...
    if (!answerText) return res.status(400).json({ message: 'No answer text or audio could be processed.' });

    const result = await createLiveTurnFeedback(session, answerText);
    saveState();
    await syncUserState(req.user, req.userData, 'live_interview_turn', { sessionId, result }, sessionId);
...
router.post('/live/complete', async (req, res) => {
  try {
    ...
    saveState();
    await syncUserState(req.user, req.userData, 'live_interview_complete', saved, saved.id);
    return res.json(result);
  } catch (error) {
