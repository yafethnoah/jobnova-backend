...
const SAFE_REALTIME_CLIENT_MESSAGE = 'Connection issue — please try to reconnect again.';
...
async function buildPreflightStatus(options = {}) {
  ...
  const checks = [
    ...
    },
    {
      ...
      ok: true,
      detail: `Client WebRTC negotiation will use ${REALTIME_CALLS_URL} with a fresh OpenAI client secret.`,
    },
    {
      id: 'ttl',
  ...
  try {
    ...
    if (!liveToken?.ok) {
      return res.status(503).json({
        message: SAFE_REALTIME_CLIENT_MESSAGE,
      });
    }
...
router.post('/session', requireAuth, async (req, res, next) => {
  try {
    ...
    const liveToken = await createRealtimeSessionToken({
      role: session.role,
    ...
    if (!liveToken?.ok) {
      ...
      session.lastError = liveToken?.error || 'Realtime client secret could not be created.';
      await saveState();
      return res.status(503).json({
        message: SAFE_REALTIME_CLIENT_MESSAGE,
        diagnostics: await buildPreflightStatus({ probe: true }),
      });
    }

    session.providerSessionId = liveToken.sessionId || null;
    session.lastTokenExpiresAt = liveToken.expiresAt || null;
    ...
    return res.json({
      ...
      transport: liveToken.transport || 'webrtc',
      model: liveToken.model || (process.env.OPENAI_REALTIME_MODEL || DEFAULT_MODEL),
      providerSessionId: session.providerSessionId,
      providerCallId: session.providerCallId,
      expiresAt: liveToken.expiresAt || null,
      connectUrl: REALTIME_CALLS_URL,
...
router.post(
  ...
  async (_req, res) => {
    return res.status(410).json({
      message:
        'This endpoint is retired. Live recruiter mode now uses direct WebRTC SDP exchange from the mobile app to OpenAI /v1/realtime/calls with a fresh client secret.',
    });
  },
);
...
router.post('/session/:id/reconnect', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!liveToken?.ok) {
      session.status = 'error';
      session.lastError = liveToken?.error || 'Realtime reconnect token could not be created.';
      await saveState();
      return res.status(503).json({ message: SAFE_REALTIME_CLIENT_MESSAGE });
    }

    session.providerSessionId = liveToken.sessionId || session.providerSessionId || null;
    session.lastTokenExpiresAt = liveToken.expiresAt || null;
    session.lastError = null;
...
router.get('/:id/feedback', requireAuth, (req, res) => {
  ...
  if (record?.feedback) {
    return res.json(record.feedback);
  }

  const activeSession = getSession(req, req.params.id);
  if (activeSession && activeSession.status !== 'ended') {
    return res.status(409).json({
      message: 'Interview analysis is generated only after the live session ends.',
    });
  }
