...
function inferQuestionFamily(role = '') {
  const normalized = String(role).toLowerCase();
  if (/hr|recruit|people|talent|employee/.test(normalized)) return 'hr';
  if (/operat|coordinator|logistic|project|admin|supply|warehouse/.test(normalized)) return 'operations';
  if (/customer|client|service|support|sales|advisor|success/.test(normalized)) return 'customer';
  return 'default';
}
...
async function completeLiveSession(sessionId, payload = {}, existingSession = null) {
  ...
  const roleFamily = inferQuestionFamily(payload.targetRole || '');
  const recurringGaps = [];
  if (scoresFromAnswers.star < 75) recurringGaps.push('STAR structure stayed inconsistent across answers.');
  if (scoresFromAnswers.clarity < 75) recurringGaps.push('Some answers were too broad before the main point became clear.');
  if (scoresFromAnswers.relevance < 80) recurringGaps.push('Role language could be matched more directly to the employer priorities.');
  if (fillerWordCount > 8) recurringGaps.push('Fillers started to affect delivery rhythm.');
  const familyPlan = {
    hr: ['Practice one employee-relations story with policy judgment and empathy.', 'Practice one recruitment or onboarding story with strong organization details.'],
    operations: ['Practice one process-improvement story with before-and-after impact.', 'Practice one prioritization story where speed and accuracy both mattered.'],
    customer: ['Practice one difficult-customer story with calm problem resolution.', 'Practice one example of rebuilding trust after a service issue.'],
    default: ['Practice one prioritization story with a measurable result.', 'Practice one problem-solving story where your action changed the outcome.']
  };
  return {
    sessionId,
    summary: `You completed a live recruiter-style interview for ${payload.targetRole || 'the target role'}. Your answers were strongest when you described your direct actions and weakest when the impact or final result stayed too vague or arrived too late in the story.`,
    strengths: ['You sounded grounded in real work rather than memorized script.', 'Your role motivation was visible.', 'You adapted across turns, which is exactly how real interview improvement works.'],
    improvementAreas: recurringGaps.length ? recurringGaps : ['Lead with your action earlier.', 'Make the result line more explicit in every answer.', 'Use one specific example for each competency instead of staying broad.'],
    ...
    nextPracticePlan: ['Rehearse a two-minute Tell me about yourself answer.', 'Practice one conflict story using STAR.', ...familyPlan[roleFamily]],
    transcript,
